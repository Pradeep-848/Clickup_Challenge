...
async def run():
    ...
    # Parse result and print a cleaner summary
    data = json.loads(result.data["custom_field"])
    print(f"✅ Custom field '{data['field_id']}' updated on Task {data['task_id']}")
    print(f"   → New Value: {data['confirmed_label']}")
